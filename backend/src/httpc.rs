...
//! `stripe.rs` are private to that module, so the Ginza agent — which talks to
//! xAI, to merchant UCP endpoints, and to Stripe's SetupIntent API — needs its
//! own client. Zero-crate: libcurl is linked as a *system library*
//! (`#[link(name = "curl")]`), exactly as `stripe.rs` links it, and every
//! `unsafe` block below carries a `// SAFETY:` justification. The exported
//! surface ([`get`], [`post_json`], [`post_form`], [`HttpResponse`]) is safe.
//!
...
//! complete body, which is all the agent needs and keeps the module small.
