...
/// A single SQLite value, covering the four storage classes this schema uses.
///
/// BLOB is deliberately absent: no column in the ported schema stores one, and
/// `sqlite3_column_text` coerces a stray blob to [`Value::Text`].
/// // yagni: no Blob variant, add one if a binary column ever lands.
...
fn read_column(stmt: &Stmt<'_>, idx: c_int) -> Value {
    ...
    // sqlite3_column_text is called before sqlite3_column_bytes, which is the
    // order SQLite documents for getting the length of the converted text, and
    // the bytes are copied out before any further call on the statement.
    unsafe {
...
// ==== BXFAV SCHEMA (app-specific) ====
//
// Saved-location CRUD. Recovered from `backend/adapters/libsql.ts` as it stood
// before the 5.6 updater deleted the JS adapter. Lives below the canonical
// block so a future `update-skateboard.js` run cannot drop it: the updater
// rewrites `db.rs` as a whole, so this must be re-applied by hand on every
// boilerplate bump. The canonical `SCHEMA` above is untouched.
const BXFAV_SCHEMA: &[&str] = &[
...
const BXFAV_INDEXES: &[&str] = &[
    "CREATE INDEX IF NOT EXISTS idx_favorites_userid ON favorites(userID)",
];

/// Columns an older `favorites` table may be missing.
const FAVORITES_COLUMNS: &[(&str, &str)] = &[
    ("userID", "TEXT NOT NULL DEFAULT ''"),
    ("title", "TEXT"),
    ("address", "TEXT"),
    ("notes", "TEXT"),
...
fn ensure_schema(db: &Db) -> Result<(), DbError> {
    ...
    for statement in BXFAV_SCHEMA {
        db.exec(statement)?;
    }
    add_missing_columns(db, "favorites", FAVORITES_COLUMNS)?;
    for statement in BXFAV_INDEXES {
        db.exec(statement)?;
    }
    Ok(())
}
...
mod tests {
    ...
    fn opens_pool_and_creates_schema() {
        ...
        assert_eq!(names, vec!["Auths", "Users", "WebhookEvents", "favorites"]);
    }
