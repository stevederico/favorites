...
fn dispatch(state: &AppState, req: &Request) -> Response {
    match (req.method.as_str(), req.path.as_str()) {
        ...
        ("GET", "/api/profiles") => profiles_get(state),
...
/// Map a `favorites` row (JSON columns as text) to the client Favorite shape.
...
fn profiles_get(state: &AppState) -> Response {
    match state.pool.with(|db| db.query("SELECT _id, name FROM Users", &[])) {
        Ok(rows) => {
        ...
        Err(e) => {
            state
                .log
                .error("Get profiles error", &[("error", json::s(e.to_string()))]);
            err_json(500, "Failed to get profiles")
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn profiles_lists_users() {
        let (state, dir) = test_state();
        signed_up(&state, "p@example.com");
        let res = handle(&state, Request::for_test("GET", "/api/profiles"));
        assert_eq!(res.status, 200);
        let body = json_body(&res);
        let arr = body.as_arr().expect("array");
        assert_eq!(arr.len(), 1);
        assert_eq!(arr[0].get_str("name"), Some("P"));
        assert!(arr[0].get_str("_id").is_some());
        std::fs::remove_dir_all(&dir).ok();
    }
}
